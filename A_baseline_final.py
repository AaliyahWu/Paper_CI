...
MODELS = {
    ...
    # LOF 的 n_neighbors 會在 run_experiment() 依 training majority 數量動態指定，
    # 與 B/C/D/E/F 的 k = min(20, n_neighbors_cap) 對齊。
    "LOF":             None,
...
def encode_labels(labels_raw, class_values=None, minority_class=None):
    """
    minority（少數類）= anomaly = 1（正類，評估目標）
    majority（多數類）= normal  = 0（負類，OCC 訓練對象）

    修正重點：
      - train fold 先判斷 minority_class。
      - test fold 必須沿用 train fold 的 minority_class，避免 train/test 各自判斷
        少數類而造成 label 反轉，導致 AUC / Recall / F1 不可比。
    """
    ...
    for ds_dir in dataset_dirs:
        ...
        for fold in range(1, N_FOLDS + 1):
            ...
            try:
                ...
                y_tst, _       = encode_labels(y_tst_raw, cv, minority_class=min_cls)

                X_train_maj = X_tra[y_tra == 0]

                if len(X_train_maj) < 5:
                    print(f"  [SKIP] Fold {fold}: 訓練集正常樣本不足 ({len(X_train_maj)})")
                    continue
            ...
            n_neighbors_cap = max(1, len(X_train_maj) - 1)
            for model_name, model_proto in MODELS.items():
                import copy
                if model_name == "LOF":
                    model = LocalOutlierFactor(
                        n_neighbors=min(20, n_neighbors_cap),
                        novelty=True,
                        contamination=0.1,
                    )
                else:
...
if __name__ == "__main__":
    print("=" * 60)
    print("Baseline A：OCC 三模型評估（OCSVM / LOF / iForest）")
    print("=" * 60)
