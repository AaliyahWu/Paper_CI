...
def run_occ_eval(occ_type, feat_maj, feat_test, y_test, n_neighbors_cap):
    ...
    OCC predict：+1=normal, -1=anomaly
    """
    scaler      = MinMaxScaler()
    ...
    if occ_type == "OCSVM":
        ...
        clf.fit(feat_maj_s)
        scores = -clf.decision_function(feat_test_s)   # higher → more anomalous
        preds  =  clf.predict(feat_test_s)              # +1 / -1

    elif occ_type == "LOF":
        k = min(20, n_neighbors_cap)
        clf = LocalOutlierFactor(n_neighbors=k, novelty=True)
        clf.fit(feat_maj_s)
        scores = -clf.decision_function(feat_test_s)
        preds  =  clf.predict(feat_test_s)
