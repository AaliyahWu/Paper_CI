...
B_baseline_grid.py
==================
對每個 dataset × fold × (AE, OCC) 枚舉全部 21 種參數組合，
所有組合結果完整保留，並另設「best」分頁組存 AUC 最高的結果。

參數搜尋空間（與 C/D/E/F 完全對齊）：
  n_layers  : [1, 2, 3]
  bottleneck: ["1/4", "1/3", "1/2", "1/1", "2/1", "3/1", "4/1"]
  → 21 種組合 per (AE, OCC, fold)

AE 類型：AE、DAE、SAE、VAE
OCC 方法：OCSVM、LOF、iForest

輸出：results/B baseline grid.xlsx
  分頁（全部組合）：
    all_per_fold      所有 21 種組合每 fold 的原始結果
    all_summary       所有組合 mean ± std（across folds，per dataset）
    all_overall       所有組合全域平均
  分頁（最佳組合）：
    best_per_fold     每 (Dataset, AE, OCC) 中『5-fold 平均 AUC 最高』的 Config，
                      保留該 Config 全部 5 fold 紀錄（per-dataset 選法）
...
OUTPUT_FILE = RESULTS_DIR / "B baseline grid.xlsx"
...
N_LAYERS_LIST     = [1, 2, 3]
BOTTLENECK_RATIOS = {
    "1/4": 0.25,   # DF 維度 = OF 維度 × 0.25
    "1/3": 1/3,    # DF 維度 = OF 維度 × 1/3
    "1/2": 0.5,    # DF 維度 = OF 維度 × 0.5
    "1/1": 1.0,    # DF 維度 = OF 維度（無壓縮，identity-size）
    "2/1": 2.0,    # DF 維度 = OF 維度 × 2（擴展）
    "3/1": 3.0,    # DF 維度 = OF 維度 × 3
    "4/1": 4.0,    # DF 維度 = OF 維度 × 4
}
ALL_CONFIGS       = [f"h{nl}-{rl}" for nl in N_LAYERS_LIST for rl in BOTTLENECK_RATIOS]
# 共 3 × 7 = 21 種組合（與 C/D/E/F 對齊）
...
def run_experiment():
    ...
    回傳：
      ...
      df_best — 每 (Dataset, AE, OCC) 中『5 fold 平均 AUC 最高』的 Config，
                保留該 Config 的全部 5 fold 紀錄（per-dataset 選法，避免 per-fold leakage）
    """
            ...
            for ae_type in AE_TYPES:
                ...
                for n_layers, ratio_label in param_configs:
                    ...
                    try:
                        ...
                        ae_features[(n_layers, ratio_label)] = (feat_maj, feat_tst)
                    except Exception as e:
                        print(f"  [ERROR] AE={ae_type} h{n_layers}-{ratio_label} Fold{fold}: {e}")

                for occ_type in OCC_TYPES:
                    for (n_layers, ratio_label), (feat_maj, feat_tst) in ae_features.items():
    ...
    # 對每個 (Dataset, AE, OCC)，從 21 個 config 中選 5-fold 平均 AUC 最高者，
    # 保留該 config 全部 5 fold 紀錄。
    df_clean = df_all.dropna(subset=["AUC"])
    best_cfg = (
        df_clean.groupby(["Dataset", "AE", "OCC", "Config"])["AUC"]
                .mean().reset_index()
                .sort_values("AUC", ascending=False)
                .drop_duplicates(["Dataset", "AE", "OCC"])
    )
    df_best = df_clean.merge(
        best_cfg[["Dataset", "AE", "OCC", "Config"]],
        on=["Dataset", "AE", "OCC", "Config"],
    )

    # 列印每個 (Dataset, AE, OCC) 的 best config
    for (ds_name, ae_type, occ_type), grp in df_best.groupby(["Dataset", "AE", "OCC"]):
        cfg = grp["Config"].iloc[0]
        auc_mean = grp["AUC"].mean()
        print(f"  [{ds_name}] {ae_type:4s}×{occ_type:8s} best={cfg:12s}  "
...
def write_summary_best(ws, df):
    """best_summary：Dataset | AE × OCC × Metric（與原版 summary 一致）"""
...
def write_overall_all(ws, df):
    ...
    c = ws["A1"]
    c.value     = "Overall Mean（all configs, all datasets & folds）- Baseline B Grid"
...
def write_overall_best(ws, df):
    ...
    ws.merge_cells(f"A1:{get_column_letter(total_cols)}1")
    c = ws["A1"]
    c.value     = "Overall Mean（best config per dataset, all datasets）- Baseline B Grid"
    c.font      = Font(name="Arial", bold=True, size=13, color="1F3864")
    c.alignment = CENTER_ALIGN
...
if __name__ == "__main__":
    print("=" * 65)
    print("Baseline B Grid Search：AE 特徵提取 × OCC（全參數搜尋）")
