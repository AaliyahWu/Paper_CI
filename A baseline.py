...
資料夾結構假設（可自行修改 DATA_ROOT）：
  data/
  └── <dataset_name>/
      ├── <dataset_name>-5-fold-tra1.dat  ... tra5.dat
      └── <dataset_name>-5-fold-tst1.dat  ... tst5.dat

KEEL .dat 格式說明：
  - 前幾行為 @attribute / @relation / @inputs / @outputs 等 header
  - @data 之後才是數值，最後一欄為 class label
  - 少數類別（minority）視為「正常」（label = 1），多數類別為異常（label = -1）
  - 本腳本自動判斷少數類為正類（訓練 OCC 只用正類）
"""
...
def encode_labels(labels_raw, class_values):
    """
    少數類為正類（+1），多數類為異常（-1）。
    若 class_values 未知，從資料中統計。
    """
    unique, counts = np.unique(labels_raw, return_counts=True)
    minority_class = unique[np.argmin(counts)]

    y = np.where(labels_raw == minority_class, 1, -1).astype(int)
...
def evaluate(model, X_train_pos, X_test, y_test):
    """訓練（只用正類）並在 test set 評估"""
    model.fit(X_train_pos)

    # decision_function：數值越高表示越像正類
    scores = model.decision_function(X_test)
    y_pred = model.predict(X_test)   # +1 / -1

    # AUC：以 decision score 計算
    try:
    ...
    for ds_dir in dataset_dirs:
        ...
        for fold in range(1, N_FOLDS + 1):
            ...
            try:
                ...
                X_train_pos = X_tra[y_tra == 1]

                if len(X_train_pos) == 0:
                    print(f"  [SKIP] Fold {fold}: 訓練集無正類樣本")
                    continue
            ...
            for model_name, model_proto in MODELS.items():
                ...
                try:
                    metrics = evaluate(model, X_train_pos, X_tst, y_tst)
                except Exception as e:
