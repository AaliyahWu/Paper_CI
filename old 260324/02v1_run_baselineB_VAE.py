"""
02v1_run_baselineB_VAE.py — Baseline B v1（VAE 版）
方法：DF_maj → VAE + OneClassSVM，5-fold 交叉驗證
評估指標：AUC / Recall(min) / F1 / G-mean

執行：python 02v1_run_baselineB_VAE.py --data_root .
輸出：./results/baseline_b_vae_v1_results.xlsx
"""
...
class VAEConfig:
    ...
    kl_weight: float    = 1e-3   # beta：KL 項的權重（β-VAE 風格）
...
def train_vae(X_maj: np.ndarray, cfg: VAEConfig, seed: int = 42):
    """只用 majority 訓練 VAE，含 early stopping（patience=15）"""
    import torch  # type: ignore
...
def run_fold(train_path: Path, test_path: Path, seed: int, vae_cfg: VAEConfig) -> Dict[str, float]:
    ...
    # 步驟 3：訓練 VAE & 提取深度特徵（mu）
    print(f"      [VAE] 訓練 (majority={Xtr_maj.shape[0]})...", end=" ", flush=True)
    vae_model, latent_dim = train_vae(Xtr_maj, vae_cfg, seed=seed)
    print(f"latent_dim={latent_dim}")

    print(f"      [VAE] 提取深度特徵 (mu)...", end=" ", flush=True)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Baseline B v1 (VAE): DF_maj → VAE + OneClassSVM")
    ...
    p.add_argument("--vae_kl_weight",    type=float, default=1e-3,
                   help="KL 散度權重 beta（β-VAE 風格），預設 0.001")
    p.add_argument("--vae_latent_ratio", type=float, default=1.0,
...
def main() -> None:
    ...
    print("=" * 70)
    print("Baseline B v1 (VAE): DF_maj → VAE + OneClassSVM")
    print(f"  VAE: epochs={vae_cfg.epochs}, batch={vae_cfg.batch_size}, "
          f"lr={vae_cfg.lr}, kl_weight={vae_cfg.kl_weight}, latent_ratio={vae_cfg.latent_ratio}")
    ...
    xlsx_path = out_dir / "baseline_b_vae_v1_results.xlsx"
