# Baseline B v2：DF_maj → OCC2 (DAE + OneClassSVM)
#
# ══════════════════════════════════════════════════════════════════════
# v2 相對於 v1 的改進項目
# ══════════════════════════════════════════════════════════════════════
#
# 【核心改進1：自適應壓縮維度】
#   v1：固定 latent_ratio=1.0（不壓縮維度）
#   v2：根據資料集 n_majority 與 IR 動態計算最佳壓縮比
#       - 若 n_maj 很小（< 50）：強壓縮 ratio=0.5（樣本少，需防過擬合）
#       - 若 n_maj 中等（50~200）：根據 IR 判斷
#           * IR > 10：中壓縮 ratio=0.5
#           * IR ≤ 10：輕壓縮 ratio=0.75
#       - 若 n_maj 大（> 200）：根據 IR 判斷
#           * IR > 20：壓縮 ratio=0.5（高 IR，minority 稀少）
#           * 5 < IR ≤ 20：輕壓縮 ratio=0.75
#           * IR ≤ 5：輕壓縮 ratio=0.75
#       目的：適應不同資料集大小與不平衡程度，降低過擬合風險
#
# 【改進2：自適應加噪強度】
#   v1：固定 noise_std=0.01
#   v2：根據 n_maj 動態調整噪聲標準差
#       - n_maj < 50：noise_std=0.05（加大噪聲，增強正則化）
#       - 50 ≤ n_maj < 200：noise_std=0.02（中等噪聲）
#       - n_maj ≥ 200：noise_std=0.01（維持小噪聲）
#       目的：樣本少時靠更強的去噪任務幫助模型泛化
#
# 【改進3：Early Stopping 自適應 patience】
#   v1：固定 patience=15
#   v2：根據 n_maj 動態調整
#       - n_maj < 50：patience=20（樣本少，loss 波動大，需更長耐心）
#       - 其他：patience=15
#       目的：小樣本資料集需要更長的訓練時間
#
# 【改進4：OCSVM nu 搜尋加入 0.2】
#   v1：nu ∈ {0.01, 0.05, 0.1}
#   v2：nu ∈ {0.01, 0.05, 0.1, 0.2}
#       原因：部分 IR 高的資料集可能需要更寬鬆的邊界
#
# 【其他改進】
#   - epochs：從 80 提高到 100（搭配 early stopping）
#   - batch_size：從 256 改為 32（更適合小資料集）
#   - weight_decay：從 1e-5 提高到 1e-4（增強正則化）
#
# 【繼承自 v1 的設計】
#   - 兩層隱藏層架構（hidden1/hidden2）
#   - LayerNorm（取代 BatchNorm）
#   - LeakyReLU(0.1)
#   - StandardScaler 只 fit majority 樣本
#   - OCSVM.predict()（無門檻搜尋）
#   - 八個資料集，5-fold 交叉驗證
#
# 執行：python 02v2_run_baselineB.py --data_root .
# 輸出：./results/baseline_b_v2_results.xlsx
# ══════════════════════════════════════════════════════════════════════
...
def adaptive_ae_params(n_maj: int, input_dim: int, ir: float) -> Tuple[float, float, int]:
    ...
    設計邏輯：
      - n_maj 小：容易過擬合 → 壓縮更多（latent↓）、雜訊更大（noise↑）、更耐心（patience↑）
      - IR 高：minority 極稀少，模型更難學到好的決策邊界 → 適度壓縮幫助泛化
    ...
    if n_maj < 50:
        ...
        if ir > 20:
            # 高 IR：minority 佔比極低，仍要壓縮以防邊界偏移
            latent_ratio = 0.5
        elif ir > 5:
            latent_ratio = 0.75
        else:
            # 低 IR（glass1, yeast1 低 IR 對照組）：輕壓縮即可
            latent_ratio = 0.75
        noise_std = 0.01
        patience  = 15
...
class AEConfig:
    ...
    # 以下三個由 adaptive_ae_params 在 run_fold 動態覆蓋（v2 新增自適應邏輯）
...
def run_fold(train_path: Path, test_path: Path, seed: int, ae_cfg: AEConfig) -> Dict[str, float]:
    ...
    Baseline B v2 單折流程：
      ...
      3. [v2] 自適應計算 latent_ratio / noise_std / patience（v1 為固定值）
      4. 訓練 DAE（只用 majority）→ 提取深度特徵
      5. 訓練集切 75/25 → 搜尋最佳 (nu, gamma)，nu 範圍加入 0.2（v1 只有 3 個值）
    ...
    # [v2] nu 加入 0.2（v1 只有 [0.01, 0.05, 0.1]）
...
# ============================================================
def parse_args():
    p = argparse.ArgumentParser(description="Baseline B v2: DF_maj → OCC2 (自適應 DAE + OCSVM)")
...
def main():
    ...
    print("=" * 70)
    print("Baseline B v2: DF_maj → OCC2（自適應 DAE 壓縮 + OCSVM）")
    print("  [v2 改進1] 自適應 latent_ratio（依 n_maj / IR 動態決定）")
    print("  [v2 改進2] 自適應 noise_std（樣本少時加大雜訊強化正則）")
    print("  [v2 改進3] OCSVM nu 搜尋加入 0.2（相對 v1 的 3 個值）")
    print("  [v2 改進4] epochs=100, batch=32, weight_decay=1e-4（v1 為 80/256/1e-4）")
    print("  [v2 改進5] Early Stopping patience 自適應（v1 固定為 15）")
    print("  [繼承 v1] LayerNorm、兩層隱藏層、StandardScaler 只 fit majority")
    print("=" * 70)
    ...
    xlsx_path = out_dir / "baseline_b_v2_results.xlsx"
