...
輸出：results/E ensemble pairs grid.xlsx
  分頁：
    ...
    best_per_fold     每 (Dataset, AE, OCC, Method, Fold) 中 AUC 最高 config 的資料
...
def run_experiment():
    ...
    # df_best：每 (Dataset, AE, OCC, Method, Fold) 中 AUC 最高的 config
    best_rows = []
    if not df_all.empty:
        for _, grp in df_all.groupby(["Dataset", "AE", "OCC", "Method", "Fold"]):
...
def write_overall_best(ws, df):
    ...
    ws.merge_cells(f"A1:{get_column_letter(total_cols)}1")
    c = ws["A1"]
    c.value     = "Overall Mean（best config per fold, all datasets）- E: Pairs Weighted Voting Grid"
    c.font      = Font(name="Arial", bold=True, size=13, color="1F3864")
    c.alignment = CENTER_ALIGN
...
def save_excel(df_all, df_best):
    ...
    ws4 = wb.create_sheet(); ws5 = wb.create_sheet(); ws6 = wb.create_sheet()
