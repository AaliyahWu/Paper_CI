# Baseline C v2：OF_maj + DF_maj → OCC3 (自適應 DAE + OCSVM)
#
# ══════════════════════════════════════════════════════════════════════
# v2 相對於 v1 的改進項目（與 Baseline B v3 的改進保持一致）
# ══════════════════════════════════════════════════════════════════════
#
# 【v1 問題診斷】
#   觀察 v1 結果發現兩個問題：
#   (1) AUC 高但 G-mean 低（如 glass-01236_vs_456: AUC=0.974, Gmean=0.477）
#       → 說明串接特徵維度翻倍後，OCSVM 超球體太緊，majority inlier 判定正常
#         但 minority 的 decision boundary 沒有切好，predict() 時邊界效果差
#   (2) yeast-05679_vs_45 的 Gmean/Recall 均下降
#       → 串接特徵讓高維空間更稀疏，OCSVM 的 gamma 選擇更難，需要更細緻調優
#
# 【核心改進】
#   [改進1] 自適應壓縮維度（與 B v3 相同的 adaptive_ae_params）
#           串接後維度 = OF(input_dim) + DF(latent_dim)
#           壓縮 DF 可控制串接特徵的總維度，避免維度爆炸
#
#   [改進2] 串接特徵的標準化處理（新增）：
#           v1：直接串接 [OF | DF]，兩者尺度可能不同
#           v2：對 DF 部分再做一次 StandardScaler（只 fit majority 的 DF）
#               確保 OF 與 DF 在相同尺度上，OCSVM 不偏重任何一部分
#
#   [改進3] OCSVM gamma 搜尋針對高維空間加密：
#           v1：gamma ∈ {scale, 1.0, 0.1, 0.01, 0.001}
#           v2：gamma ∈ {scale, 0.5, 0.1, 0.05, 0.01, 0.001}
#           增加 0.5 和 0.05，對串接高維特徵提供更細緻的 RBF 寬度選擇
#
#   [改進4] nu 搜尋加入 0.2（與 B v3 一致）
#
#   [改進5] 自適應 noise_std 和 patience（與 B v3 一致）
#
#   [改進6] batch_size=32（適合小資料集）
#
# 執行：python 03v2_run_baselineC.py --data_root .
# 輸出：./results/baseline_c_v2_results.xlsx
# ══════════════════════════════════════════════════════════════════════
...
# ============================================================
# [v2 改進1] 自適應超參數（與 B v3 相同邏輯）
# ============================================================
def adaptive_ae_params(n_maj: int, input_dim: int, ir: float) -> Tuple[float, float, int]:
    """
    根據 n_maj 和 IR 動態決定 (latent_ratio, noise_std, patience)

    設計邏輯（與 Baseline B v2 相同邏輯）：
      - n_maj 小：容易過擬合 → 壓縮更多、雜訊更大、更耐心
      - IR 高：minority 極稀少 → 適度壓縮幫助泛化
      
    特別考量（針對 Baseline C 串接特徵維度翻倍）：
      - 串接後維度 = input_dim + latent_dim
      - latent_ratio 越小 → 串接總維度越小 → OCSVM 在高維空間更穩定
      - 因此 C 可以考慮相同或稍微積極的壓縮策略
    """
    if n_maj < 50:
        # 樣本極少：強壓縮 + 大噪聲 + 長耐心
        latent_ratio = 0.5
...
def run_fold(train_path: Path, test_path: Path, seed: int, ae_cfg: AEConfig) -> Dict[str, float]:
    ...
    Baseline C v2 單折流程：
      ...
      3. [v2] 自適應計算 latent_ratio / noise_std / patience
    ...
    # [v2 改進2] DF 標準化（獨立的 scaler，只 fit majority DF）
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Baseline C v2: OF_maj + DF_maj → OCC3（自適應 DAE + OCSVM）"
    )
...
def main():
    ...
    print("=" * 70)
    print("Baseline C v2: OF_maj + DF_maj → OCC3（自適應 DAE concat OCSVM）")
    ...
    print("  [v2 改進4] OCSVM nu 加入 0.2（高 IR 資料集需要更鬆散邊界）")
    print("  [繼承 v1] LayerNorm、兩層隱藏、early stopping、只 fit majority")
    print("=" * 70)
    ...
    xlsx_path = out_dir / "baseline_c_v2_results.xlsx"
