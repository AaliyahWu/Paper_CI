...
每個 fold 流程：
  1. 載入 KEEL .dat
  2. MinMaxScale 原始特徵（與 Baseline B 一致）
  3. 用最佳 AE 架構（來自 best_params.csv）對 X_maj_scaled 訓練
  4. 提取深度特徵 feat_maj, feat_test
  5. 拼接原始特徵 + 深度特徵：
       X_comb_maj  = hstack([X_maj_scaled,  feat_maj])
       X_comb_test = hstack([X_tst_scaled,  feat_test])
  6. 對拼接後特徵再做 MinMaxScale（確保 OCC 輸入尺度一致）
  7. 訓練 OCC → 評估 AUC、F1、Recall、G-mean
...
def run_occ_eval(occ_type, X_comb_maj, X_comb_test, y_test, n_neighbors_cap):
    """
    在拼接特徵空間（OF_maj + DF_maj）訓練 OCC（只用 majority/normal 拼接特徵），
    對 test set 計算 AUC、F1、Recall、G-mean。

    y_test  : 1=minority(anomaly), 0=majority(normal)

    ▸ 核心差異（相對於 Baseline B）：
      輸入的 X_comb_maj / X_comb_test 已是原始特徵（OF_maj）與
      AE 深度特徵（DF_maj）水平拼接後的矩陣。
      此處再做一次 MinMaxScale 以確保合併後各維度尺度一致。
    ...
    # Threshold：training majority anomaly score 的第 90 百分位數（與 A、B 一致）
...
def run_experiment(best_params_map):
    """
    對每個 dataset × fold × (AE, OCC) 組合：
      1. MinMaxScale 原始特徵
      2. 訓練 AE (best params)，提取深度特徵
      3. 拼接 OF_maj + DF_maj（訓練集 majority 及測試集）
      4. OCC 訓練 + 評估
    """
    ...
    for ds_dir in dataset_dirs:
        ...
        for fold in range(1, N_FOLDS + 1):
            ...
            try:
                ...
                # ── Step 1: MinMaxScale 原始特徵（fit on X_maj, transform X_tst）──
            ...
            for ae_type in AE_TYPES:
                for occ_type in OCC_TYPES:
                    ...
                    try:
                        ...
                        # ── Step 3 (核心): 拼接原始特徵 + 深度特徵 ──
                        #   OF_maj  shape: (n_maj, input_dim)
                        #   DF_maj  shape: (n_maj, n_units)
                        #   Combined shape: (n_maj, input_dim + n_units)
                        X_comb_maj  = np.hstack([X_maj_s,  feat_maj])   # train: majority only
                        X_comb_test = np.hstack([X_tst_s,  feat_tst])   # test: all samples

                        # ── Step 4: OCC 訓練 + 評估（run_occ_eval 內含重新縮放）──
                    ...
                    print(
                        f"  {ae_type:4s}×{occ_type:8s}({cfg_label}) Fold{fold}  "
...
# ─────────────────────────── Excel 樣式定義 ──────────────────────────────────
HEADER_FILL = PatternFill("solid", fgColor="2F5597")   # 深藍（與 A/B 一致）
...
def write_summary(ws, df):
    ...
    # Row 1: Dataset (span 2 rows) | AE groups
...
def write_overall_mean(ws, df):
    ...
    title = ws["A1"]
    title.value     = "Overall Mean（across all datasets & folds）- Baseline C: OF + DF → OCC"
...
if __name__ == "__main__":
    print("=" * 65)
    print("Baseline C：OF_maj + DF_maj → OCC3")
