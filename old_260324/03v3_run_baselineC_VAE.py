# Baseline C v3：OF_maj + DF_maj → OCC3 (自適應 VAE + OCSVM)
#
# ══════════════════════════════════════════════════════════════════════
# v3 相對於 v2 的改進項目（與 Baseline B v3 VAE / C v3 DAE 保持一致）
# ══════════════════════════════════════════════════════════════════════
#
# 【核心改進：自適應壓縮維度強化，同步 B v3 VAE】
#   v2：只有 0.5 和 0.75 兩種壓縮比
#   v3：新增 ratio=1.0（不壓縮），根據 IR 更精細化調整
#       - 若 n_maj < 50：ratio=0.5
#       - 若 n_maj 中等（50~200）：IR > 10 → 0.5 | 5 < IR ≤ 10 → 0.75 | IR ≤ 5 → 1.0 [v3]
#       - 若 n_maj 大（> 200）：IR > 20 → 0.5 | 5 < IR ≤ 20 → 0.75 | IR ≤ 5 → 1.0 [v3]
#   DF 標準化（v2 已加入）確保 OF 與 DF 同尺度，控制串接維度影響
#
# 【改進2：nu 搜尋範圍回歸 {0.01, 0.05, 0.1}（同 B v3）】
#
# 【繼承 v2 的所有改進】
#   - DF 獨立標準化 normalize_df()
#   - OCSVM gamma 搜尋 {scale, 0.5, 0.1, 0.05, 0.01, 0.001}（6 個值）
#   - 自適應 kl_weight 和 patience
#   - epochs=100, batch=32, weight_decay=1e-4
#   - LayerNorm、兩層隱藏層
#
# 執行：python 03v3_run_baselineC_VAE.py --data_root .
# 輸出：./results/baseline_c_vae_v3_results.xlsx
# ══════════════════════════════════════════════════════════════════════
...
# ============================================================
# [v3 改進1] 自適應超參數計算（VAE 版，新增 ratio=1.0，同步 B v3 VAE）
# ============================================================
def adaptive_vae_params(n_maj: int, input_dim: int, ir: float) -> Tuple[float, float, int]:
    ...
    v3 改進：新增 ratio=1.0 用於低不平衡資料集（IR ≤ 5），同步 C v3 DAE / B v3 VAE
    DF 標準化（v2 已加入）確保 OF 與 DF 同尺度，控制串接後的維度影響

    latent_ratio：決定 DF 維度 → 影響串接特徵總維度
    kl_weight：對應 DAE 的 noise_std（正則化強度）
    """
    if n_maj < 50:
        # 樣本極少：強壓縮 + 大 KL 權重 + 長耐心
        latent_ratio = 0.5
        kl_weight    = 5e-3
        patience     = 20
    elif n_maj < 200:
...
# ============================================================
# VAE 模型（架構與 B v3 VAE / C v2 VAE 相同）
# ============================================================
...
def normalize_df(
    ...
    """[v2 繼承] DF 標準化：只 fit majority DF，確保 OF 與 DF 同尺度"""
    scaler = StandardScaler()
...
def run_fold(train_path: Path, test_path: Path, seed: int, vae_cfg: VAEConfig) -> Dict[str, float]:
    """
    Baseline C v3 (VAE) 單折流程：
      1. 讀資料
      2. OF 標準化（scaler 只 fit majority）
      3. [v3] 自適應計算 latent_ratio / kl_weight / patience（新增 ratio=1.0）
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Baseline C v3 (VAE): OF_maj + DF_maj → OCC3（自適應 VAE concat OCSVM）"
    )
...
def main():
    ...
    print("=" * 70)
    print("Baseline C v3 (VAE): OF_maj + DF_maj → OCC3（自適應 VAE concat OCSVM）")
    print("  [v3 改進1] 自適應 latent_ratio 新增 ratio=1.0（低不平衡資料集，同 B v3 VAE）")
    print("    - ratio=1.0：IR ≤ 5 時不壓縮，保留完整 DF 特徵空間")
    ...
    xlsx_path = out_dir / "baseline_c_vae_v3_results.xlsx"
